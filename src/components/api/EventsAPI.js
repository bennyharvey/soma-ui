...
export const gett = ({page = 1, perPage = 10, filterType}) => basicRequest({
    url: config.NEW_EVENTS_URL +'?per-page=' + perPage + ('&page=' + (page - 1) * 10) + (filterType ? '?filter[type]=' + filterType : '')
})
